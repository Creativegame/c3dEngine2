...
void initGame(){
	...
	//----fbx model
    Cc3dSkinActor*actor=Cc3dFbxLoader::sharedFbxLoader()->load("fbxLoader_resource/caomei/caomei.fbx",1.0f/10);//"fbxLoader_resource/girl/girl.fbx",1.0f/10);
	
	actor->setPos(Cc3dVector4(0,0,0,1));
	actor->setLight(light0);
    actor->setCamera(camera);
	actor->setProgram(Cc3dProgramCache::sharedProgramCache()->getProgramByName("shader_texColorOnly"));
    actor->setPassUnifoCallback(buildinProgramPassUnifoCallback_texColorOnly);
